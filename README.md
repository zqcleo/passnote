...
构建完成后会得到一个 APK 文件，可直接安装到 Android 设备。

### 本地构建（无需 Expo 账号）

如果想本地构建，需要配置 Android 开发环境：
...
1. 打开应用时会显示锁屏界面
2. 点击"使用生物识别解锁"按钮，使用指纹/Face ID 解锁
...
**Q: 如何在真机上安装？**
A: 使用 Expo Go 扫码预览，或构建 APK 后传输到手机安装。

**Q: 数据会丢失吗？**
A: 数据存储在设备本地，卸载应用会清除数据，建议定期导出备份（后续版本可添加）。

**Q: 支持指纹解锁吗？**
A: 支持！移动端支持指纹/Face ID 生物识别解锁和手势密码解锁。
